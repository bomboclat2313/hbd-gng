...
// Allow parent window to enable sound automatically via postMessage
window.addEventListener('message', (event) => {
	const data = event && event.data;
	if (!data || typeof data !== 'object') return;
	if (data.type === 'enable-sound') {
		const enable = data.enabled !== false;
		if (enable) {
			try { soundManager.ctx.resume && soundManager.ctx.resume(); } catch (e) {}
		}
		// Ensure app is unpaused and sound state is set
		toggleSound(enable);
		// Start custom sequence if provided (only once) when enabling sound.
		if (enable && Array.isArray(data.customSequence) && data.customSequence.length && !customIntroStarted) {
			customIntroStarted = true;
			startCustomSequence(data.customSequence);
		}
		if (enable) {
			togglePause(false);
		}
...
// Custom sequence controller - runs fireworks in a specific order
let customSequenceQueue = [];
let customSequenceRunning = false;
let customSequenceCooldownUntil = 0;
let customIntroStarted = false;

function isCustomSequenceActive() {
	return customSequenceRunning || Date.now() < customSequenceCooldownUntil;
}

function startCustomSequence(sequenceConfig) {
	// Skip default first-sequence intro and continue with normal random after custom intro.
	isFirstSeq = false;
	customSequenceQueue = sequenceConfig || [];
	customSequenceRunning = true;
	customSequenceCooldownUntil = 0;
	runNextInCustomSequence();
}

function runNextInCustomSequence() {
	if (!customSequenceRunning || customSequenceQueue.length === 0) {
		customSequenceRunning = false;
		// Small cooldown so random auto-fire never overlaps the end of the final combo burst.
		customSequenceCooldownUntil = Date.now() + 1400;
		return;
	}
	
	const step = customSequenceQueue.shift();
	const { type, count = 1, size, delay = 0 } = step;
	...
	setTimeout(() => {
		...
		switch(type) {
			case 'single':
				sequenceDelay = seqRandomShell();
				break;
			case 'double':
				sequenceDelay = seqTwoRandom();
				break;
			case 'triple':
				sequenceDelay = seqTriple();
				break;
			case 'pyramid':
				sequenceDelay = seqPyramid();
				break;
			case 'barrage':
				sequenceDelay = seqSmallBarrage();
				break;
			case 'burst':
				// Rapid fire burst
				for (let i = 0; i < count; i++) {
					setTimeout(() => seqRandomFastShell(), i * 150);
				}
				sequenceDelay = count * 150 + 2000;
				break;
			default:
				sequenceDelay = 1000;
		}
		
		// Continue to next step after current sequence completes
		setTimeout(() => runNextInCustomSequence(), sequenceDelay);
	}, delay);
}
...
function init() {
	...
	// Begin simulation - but stay paused if loaded in iframe (parent will control this)
	const isInIframe = window.self !== window.top;
	if (!isInIframe) {
		togglePause(false);
	}
	// If in iframe, stay paused until parent sends enable-sound message
...
function seqPyramid() {
	...
	function launchShell(x, useSpecial) {
		const isRandom = shellNameSelector() === 'Random';
		const boundedX = fitShellPositionInBoundsH(x);
		...
		const height = boundedX <= 0.5 ? boundedX / 0.5 : (1 - boundedX) / 0.5;
		shell.launch(boundedX, useSpecial ? 0.75 : height * 0.42);
	}
...
function updateGlobals(timeStep, lag) {
	...
	// auto launch shells
	if (store.state.config.autoLaunch && !isCustomSequenceActive()) {
